	...
	<script>
		$(document).ready( function () {
			...
			$('.editar').on('click',function(e){
 				...
 				document.getElementById("lesson_id").value = $(this).attr('data-id');
				document.getElementById("lesson_title").value = $(this).attr('data-title');
				 document.getElementById("lesson_description").value = $(this).attr('data-description');
				 document.getElementById("lesson_url").value = $(this).attr('data-url');
						...
						@foreach($curso->lessons as $leccion)
							<tr>
								...
								<td class="text-center">
									<a class="btn btn-info editar" data-id="{{ $leccion->id }}" data-title="{{ $leccion->title }}" data-description="{{ $leccion->description }}" data-url="{{ $leccion->url }}"><i class="fa fa-edit"></i></a>
									<!--<a class="btn btn-warning" href="#" title="Ver Recursos Adicionales"><i class="fa fa-search"></i></a>
				    ...
				    <div class="modal-body">
						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class="form-group">
									<label>Título</label>
									<input type="text" class="form-control" name="title" id="lesson_title" required>
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
									<label>Descripción</label>
									<textarea class="form-control" name="description" id="lesson_description"></textarea> 
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group">
									<label>URL</label>
									<input type="url" class="form-control" name="url" id="lesson_url" required>
								</div>
							</div>
						</div>
						</div>
